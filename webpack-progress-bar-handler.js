...
module.exports = function (options) {
    ...
    var handler = function (percentage, message) {
        ...
        output(options.label + '[' + (new Array(arrowLength).join(options.arrowBody)) + options.arrowHead + (new Array(options.progressLength - arrowLength).join('-')) + '] ' + (percentage * 100).toFixed(2) + '% : ' + message);
    };
